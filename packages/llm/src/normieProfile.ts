...
Return STRICT JSON with fields: name (a short, everyday human name) and description (one short sentence about their lifestyle that matches how healthy they are).
Mention the name in the description. Keep it under 25 words.
  `.trim();

  const userPrompt = `
Health scale is 15 (very unhealthy) to 100 (peak fitness).
Provided health: ${health}.
Examples:
- 15 -> "John likes to Netflix and chill and only eats burgers."
